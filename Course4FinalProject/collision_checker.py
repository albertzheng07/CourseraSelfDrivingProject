    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                circle_locations[:, 0] =  [path[0][j] + offset*cos(path[2][j]) for offset in self._circle_offsets]
                circle_locations[:, 1] =  [path[1][j] + offset*sin(path[2][j]) for offset in self._circle_offsets]
                # --------------------------------------------------------------
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                score = np.sum([np.linalg.norm(np.array(goal_state[:2])-np.array([x, y])) \
                                                 for x, y in zip(paths[i][0],paths[i][1])])
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            dist_sum_to_coll_path = 0.0
                            for x, y in zip(paths[i][0], paths[i][1]):
                                closest_len, _  = get_closest_index(paths[j], [x, y])
                                dist_sum_to_coll_path += closest_len
                            
                            score += self._weight * 1.0 / dist_sum_to_coll_path                                 
                            # --------------------------------------------------
...
def get_closest_index(waypoints, point):
    ...
    for i in range(len(waypoints)):
        dist_to_waypoint = np.linalg.norm(np.array(waypoints[i][:2])-np.array(point))
