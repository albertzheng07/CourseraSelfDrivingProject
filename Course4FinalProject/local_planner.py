...
class LocalPlanner:
    ...
    def get_goal_state_set(self, goal_index, goal_state, waypoints, ego_state):
        ...
        # ------------------------------------------------------------------
        front_index = goal_index + 1
        rear_index = goal_index        
        if (goal_index >= len(waypoints) - 1): 
            front_index = goal_index
            rear_index = goal_index - 1     
            
        delta_x = waypoints[front_index][0] - waypoints[rear_index][0] 
        delta_y = waypoints[front_index][1] - waypoints[rear_index][1]            
        ...
        goal_vec = np.matmul(rot_mat,np.array([goal_state_local[0],goal_state_local[1]]))
        goal_x = goal_vec[0]
        goal_y = goal_vec[1]
        # ------------------------------------------------------------------
        ...
        if self._prev_best_path == None:
            _, closest_idx = get_closest_index(waypoints, ego_state)
            self._prev_best_path = waypoints[closest_idx:front_index+1]
        ...
        for i in range(self._num_paths):
            ...
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            x_offset = offset*cos(goal_t + pi /2) # projection perpendicular to path with same heading
